...
def _generate_pdf(report: VehicleReportOut, start: date, end: date) -> bytes:
    ...
    other_total = (report.other_expenses or 0.0) + (report.extra_expenses or 0.0)
    ...
    pdf.cell(0, 12, "SMITHSONS LOGISTICS", ln=True, align="C")

    pdf.set_font("Helvetica", "", 14)
    pdf.cell(0, 10, "Trip Earnings Payslip", ln=True, align="C")

    pdf.ln(6)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 8, f"{generated_on} - Generated by Easy Estates", ln=True, align="C")
    ...
    if report.actual_payment is not None:
        rows.append(("Actual Payment", _format_currency(report.actual_payment)))
...
@router.get("/pdf/{vehicle_id}")
def download_vehicle_report_pdf(
    vehicle_id: int,
    start_date: Optional[date] = Query(None),
    end_date: Optional[date] = Query(None),
    db: Session = Depends(get_db),
